...
where
    T: MixedNum + MixedNumConversion<i32> + MixedOps,
{
    if power == 0 {
...
pub fn sin<T>(x: T) -> T
where
    T: fixed::traits::FixedSigned + MixedNum + MixedNumConversion<i32> + MixedOps,
{
...
pub fn cos<T>(x: T) -> T
where
    T: fixed::traits::FixedSigned + MixedNum + MixedNumSigned + MixedOps + MixedPi,
{
