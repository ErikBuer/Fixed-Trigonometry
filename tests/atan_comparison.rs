...
fn atan_comparison() -> Result<(), Box<dyn std::error::Error>> 
{
    ...
    let atan2_series = LineSeries::new(
        (-500..=500).map(|x| x as f32 *PI / 500.0).map(|x| (x, atan::atan2(F::<U>::from_num(x), F::<U>::from_num(1) ).to_num::<f32>() )),
        &RED);
    
    let atan2_fast_series = LineSeries::new(
        (-500..=500).map(|x| x as f32 *PI / 500.0).map(|x| (x, atan::atan2_fast(F::<U>::from_num(x), F::<U>::from_num(1) ).to_num::<f32>() )),
        &CYAN);
    
    let std_atan_series = LineSeries::new(
        (-500..=500).map(|x| x as f32 *PI / 500.0).map(|x| (x, x.atan() )),
        &GREEN);
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .build_cartesian_2d( -PI..PI, -1.3f32..1.3f32)?;
