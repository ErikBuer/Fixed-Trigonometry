...
fn compare_sqrt() -> Result<(), Box<dyn std::error::Error>> 
{
    ...
    chart
        .draw_secondary_series(LineSeries::new(
            (-500..=500).map(|x| x as f32 / 500.0).map(|x| (x, f32::abs(x.sqrt()-sqrt::niirf(F::<U>::from_num(x), 2).to_num::<f32>()) )),
            &BLUE,
        ))?
        .label("error")
...
fn compare_error_sqrt() -> Result<(), Box<dyn std::error::Error>> 
{
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .y_label_area_size(30)
        .right_y_label_area_size(60)
        .build_cartesian_2d( 0.001f32..1f32,  0f32..1f32)?
        .set_secondary_coord( 0.001f32..1f32, 0f32..1f32);
    ...
    chart
        .draw_series(LineSeries::new(
            (-100..=100).map(|x| x as f32 / 100.0).map(|x| (x, f32::abs(x.sqrt()-sqrt::niirf(F::<U>::from_num(x), 2).to_num::<f32>()) )),
            &BLUE,
        ))?
        .label("error")
        .legend(|(x, y)| PathElement::new(vec![(x, y), (x + 20, y)], &BLUE));
    ...
    chart
        .draw_secondary_series(LineSeries::new(
            (-100..=100).map(|x| x as f32 / 100.0).map(|x| (x, f32::abs(x.sqrt()-cordic::sqrt(F::<U>::from_num(x)).to_num::<f32>()) )),
            &BLUE,
        ))?
        .label("error")
        .legend(|(x, y)| PathElement::new(vec![(x, y), (x + 20, y)], &BLUE));
